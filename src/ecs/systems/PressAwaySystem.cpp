...
PressAwaySystem::PressAwaySystem(entt::registry& registry) : IUpdateSystem(registry) {
}
...
void PressAwaySystem::update(float dt) {
	...
	entities.each([this, &entities](auto entity, auto& pos, auto& vel, auto& presser) {
		glm::vec3 mean_vel(0.0f);
		int count = 0;
		entities.each([this, entity, &pos, &vel, &presser, &mean_vel, &count](auto other_entity, auto& other_pos,
		                                                                      auto& other_vel, auto& other_presser) {

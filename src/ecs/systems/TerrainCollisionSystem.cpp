...
TerrainCollisionSystem::TerrainCollisionSystem(entt::registry& registry, std::shared_ptr<ChunkedWorld> chunkedWorld)
    : IUpdateSystem(registry), m_chunkedWorld{chunkedWorld} {
}

TerrainCollisionSystem::~TerrainCollisionSystem() {
}

void TerrainCollisionSystem::update(float dt) {
	registry.view<CPosition, CVelocity, CTerrainCollider>().each([this](auto entity, auto& pos, auto& vel,
		...
		float to_dir = m_chunkedWorld->raycastd(pos.pos + glm::vec3(0.f, 0.02f, 0.f),
		                                        vel.vel * glm::vec3(1.f, 0.f, 1.f) * 0.1f, 0.1f);

		glm::vec3 maxStairEnd = pos.pos + vel.vel;
		glm::vec3 maxStairStart = maxStairEnd + glm::vec3(0.f, collider.max_stair_height, 0.f);
		float to_stair = m_chunkedWorld->raycastd(maxStairStart, glm::normalize(maxStairEnd - maxStairStart) * 0.001f,

...
GravitySystem::GravitySystem(entt::registry& registry, float gravity) : IUpdateSystem(registry), gravity(gravity) {
}

void GravitySystem::update(float dt) {
	registry.view<CPosition, CVelocity, CGravity>().each(
	    [this, &registry = registry](auto entity, auto& pos, auto& vel, auto& gravity) {

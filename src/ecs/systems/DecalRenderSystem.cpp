...
void DecalRenderSystem::initCube() {
	GLfloat vertices[] = {-.5f, -.5f, -.5f, .5f, -.5f, -.5f, -.5f, .5f, -.5f, .5f, .5f, -.5f,
...
void DecalRenderSystem::collectInstanceData() {
	...
	cregistry.view<const CPosition, const COrientation, const CDecal>().each(
	    [this](const auto& pos, const auto& orient, const auto& decal) {
		    glm::mat4 model = glm::scale(glm::rotate(glm::translate(glm::mat4(1.f), pos.pos),
		                                             orient.amount * glm::pi<float>() * 2.f, orient.orientation),
		                                 decal.size);
		    ...
		    if (exists == m_boundTextures.end()) { // texture is not bound
			    ...
			    std::string uniformName =
			        (std::string("uTextures[") + std::to_string(m_boundTextures.size()) + std::string("]"));
	...
	glBufferSubData(GL_ARRAY_BUFFER, m_aInstanceTexOffsets.size() * sizeof(glm::vec4),
	                m_aInstanceModels.size() * sizeof(glm::mat4), m_aInstanceModels.data());
	glBufferSubData(GL_ARRAY_BUFFER,
	                m_aInstanceTexOffsets.size() * sizeof(glm::vec4) + m_aInstanceModels.size() * sizeof(glm::mat4),
	                m_aInstanceTextures.size() * sizeof(GLuint), m_aInstanceTextures.data());
	...
	for (GLuint i = 0; i < 4; ++i) {
		glVertexAttribPointer(attribInstanceModel + i, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
		                      (void*)(m_aInstanceTexOffsets.size() * sizeof(glm::vec4) + i * sizeof(glm::vec4)));
		glEnableVertexAttribArray(attribInstanceModel + i);
		glVertexAttribDivisor(attribInstanceModel + i, 1);
	}

	glVertexAttribIPointer(
	    attribTextureIndex, 4, GL_UNSIGNED_INT, sizeof(GLuint),
	    (void*)(m_aInstanceTexOffsets.size() * sizeof(glm::vec4) + m_aInstanceModels.size() * sizeof(glm::mat4)));

...
void WayfindSystem::update(float dt) {
	registry.view<CPosition, CRunningToTarget>().each(
	    [&registry = registry](auto entity, auto& pos, auto& runToTarget) {
...
void WayfindSystem::draw() {
	...
	registry.view<CPosition, CRunningToTarget>().each(
	    [&registry = registry, &lines](auto entity, auto& pos, auto& runToTarget) {

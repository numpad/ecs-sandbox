...
std::vector<Vertex> CubeMarcher::polygonize(const Terrain& terrain) {
	...
	vec3 min = ppmin / stepscale, max = ppmax / stepscale;
...
int CubeMarcher::polygonizeCube(const Terrain& terrain, vec3 cellStart, std::vector<Vertex>& triangleVertices) {
	CubeMarcher::Cell cell = getCell(terrain, cellStart);

	int cubeindex, edges;
	if (!getEdges(cell, &cubeindex, &edges))
		return 0; // Cube/Cell is fully in/outside the surface

	constexpr int MAX_VERTICES = 12;
	vec3 vertices[MAX_VERTICES];

	if (edges & 1)
		vertices[0] = interpolate(cell.points[0], cell.points[1], cell.values[0], cell.values[1]);
	if (edges & 2)
		vertices[1] = interpolate(cell.points[1], cell.points[2], cell.values[1], cell.values[2]);
	if (edges & 4)
		vertices[2] = interpolate(cell.points[2], cell.points[3], cell.values[2], cell.values[3]);
	if (edges & 8)
		vertices[3] = interpolate(cell.points[3], cell.points[0], cell.values[3], cell.values[0]);
	if (edges & 16)
		vertices[4] = interpolate(cell.points[4], cell.points[5], cell.values[4], cell.values[5]);
	if (edges & 32)
		vertices[5] = interpolate(cell.points[5], cell.points[6], cell.values[5], cell.values[6]);
	if (edges & 64)
		vertices[6] = interpolate(cell.points[6], cell.points[7], cell.values[6], cell.values[7]);
	if (edges & 128)
		vertices[7] = interpolate(cell.points[7], cell.points[4], cell.values[7], cell.values[4]);
	if (edges & 256)
		vertices[8] = interpolate(cell.points[0], cell.points[4], cell.values[0], cell.values[4]);
	if (edges & 512)
		vertices[9] = interpolate(cell.points[1], cell.points[5], cell.values[1], cell.values[5]);
	if (edges & 1024)
		vertices[10] = interpolate(cell.points[2], cell.points[6], cell.values[2], cell.values[6]);
	if (edges & 2048)
		vertices[11] = interpolate(cell.points[3], cell.points[7], cell.values[3], cell.values[7]);

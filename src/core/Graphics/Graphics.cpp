...
void Graphics::setDepthTest(const bool new_depth_test) {
	if (m_currentGlState.depth_test == new_depth_test)
		return;
...
void Graphics::setDepthWrite(const bool new_depth_write) {
	if (m_currentGlState.depth_write == new_depth_write)
		return;
...
void Graphics::setCullFace(const bool new_cull_face) {
	if (m_currentGlState.cull_face == new_cull_face)
		return;
...
void Graphics::setBlend(const bool new_blend) {
	if (m_currentGlState.blend == new_blend)
		return;
...
void Graphics::setBlendFunc(const int src, const int dst, const int src_alpha, const int dst_alpha) {
	if (m_currentGlState.blend_src == src && m_currentGlState.blend_dst == dst &&
	    m_currentGlState.blend_src_alpha == src_alpha && m_currentGlState.blend_dst_alpha == dst_alpha)
		return;
	...
	glBlendFuncSeparate(m_currentGlState.blend_src, m_currentGlState.blend_dst, m_currentGlState.blend_src_alpha,
	                    m_currentGlState.blend_dst_alpha);
}

...
float raycast(ISignedDistanceFunction& sdf, vec3 origin, vec3 dir, float max_length) {
	...
	if (dist < max_length) {
		if (sdf.get_distance(origin + dir * max_length) <= epsilon)
			return max_length;
	}
	return -1.0f;
}
...
imat2x3 get_affected_chunks(mat2x3 aabb, vec3 chunksize) {
	return imat2x3(ivec3(floor(aabb[0] / chunksize)), ivec3(ceil(aabb[1] / chunksize)) - 1);
}
...
int solve_ballistic_arc(vec3 proj_pos, float proj_speed, vec3 target, float gravity, vec3& s0, vec3& s1) {
	...
	//   (10) p = (v^2 +- sqrt(v^4 - g(g*x^2 + 2*y*v^2)))/gx        [multiply top/bottom by -2*v*v/x; move 4*v^4/x^2
	//   into root] (11) O = atan(p)
...
float ballistic_range(float speed, float gravity, float initial_height) {
	...
	//   (3) via quadratic: t = (speed*sin O)/gravity + sqrt(speed*speed*sin O + 2*gravity*initial_height)/gravity
	//   [ignore smaller root] (4) solution: range = x = (speed*cos O)/gravity * sqrt(speed*speed*sin O +
	//   2*gravity*initial_height)	[plug t back into x=speed*time*cos O]
	float angle = radians(45.0f); // no air resistence, so 45 degrees provides maximum range
	float cosp = cos(angle);
	float sinp = sin(angle);

	float range =
	    (speed * cosp / gravity) * (speed * sinp + sqrt(speed * speed * sinp * sinp + 2.0f * gravity * initial_height));
	return range;
}

bool solve_ballistic_arc_lateral(vec3 proj_pos, float lateral_speed, vec3 target_pos, float max_height,
                                 vec3& fire_velocity, float& gravity) {
...
float lightVolumeRadius(vec3 lightColor, float constant, float linear, float quadratic) {
	...
	float radius = (-linear + std::sqrt(linear * linear - 4.0f * quadratic * (constant - (256.0f / 5.0f) * lightMax))) /
	               (2.0f * quadratic);

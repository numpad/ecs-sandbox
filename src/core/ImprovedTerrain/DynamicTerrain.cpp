...
float DynamicTerrain::signed_distance_function(glm::vec3 world_pos) const {
	...
	if (sdf)
		return sdf->get_distance(world_pos);
...
void DynamicTerrain::polygonize(glm::ivec3 chunk_idx) {
	...
	glm::vec3 min = glm::vec3(chunk_idx.x * m_chunksize.x, chunk_idx.y * m_chunksize.y, chunk_idx.z * m_chunksize.z);
	glm::vec3 max = glm::vec3(chunk_idx.x * m_chunksize.x + m_chunksize.x, chunk_idx.y * m_chunksize.y + m_chunksize.y,

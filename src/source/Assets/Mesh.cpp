...
void Mesh::setupMesh() {
	...
	glBufferData(GL_ARRAY_BUFFER, rawvertices.size() * sizeof(GLfloat), &rawvertices[0], GL_STATIC_DRAW);
	if (hasIndices) {
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, rawvertexsize, (void*)offsetof(Vertex, position));
	// normal

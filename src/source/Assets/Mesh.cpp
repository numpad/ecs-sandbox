...
Mesh::Mesh(std::vector<Vertex> vertices, bool hasTexcoords)
	: vertices(vertices), hasTexcoords(hasTexcoords),
	hasIndices(false)
{
	setupMesh();
}
...
Mesh::Mesh(std::vector<Vertex> vertices, std::vector<Texture *> textures,
	bool hasTexcoords)
	: vertices(vertices), textures(textures), hasTexcoords(hasTexcoords),
	hasIndices(false)
{
	setupMesh();
}
...
Mesh::Mesh(std::vector<Vertex> vertices, std::vector<GLuint> indices,
	bool hasTexcoords)
	: vertices(vertices), indices(indices), hasTexcoords(hasTexcoords),
	hasIndices(true)
{
	setupMesh();
}
...
Mesh::Mesh(std::vector<Vertex> vertices, std::vector<GLuint> indices,
	       std::vector<Texture *> textures, bool hasTexcoords)
	: vertices(vertices), indices(indices), textures(textures),
	  hasTexcoords(hasTexcoords), hasIndices(true)
{
	setupMesh();
}
...
void Mesh::setupMesh() {
	...
	size_t rawvertexsize = sizeof(Vertex) - sizeof(vec2);
	if (hasTexcoords) {
		rawvertexsize += sizeof(vec2);
	}
	...
	glBindVertexArray(VAO);
		...
		glBufferData(GL_ARRAY_BUFFER, rawvertices.size() * sizeof(GLfloat),
			&rawvertices[0], GL_STATIC_DRAW);
		if (hasIndices) {
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, rawvertexsize,
			(void *)offsetof(Vertex, position));
		// normal
		glEnableVertexAttribArray(1);
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, rawvertexsize,
			(void *)offsetof(Vertex, normal));
		// texcoord
		if (hasTexcoords) {
			glEnableVertexAttribArray(2);
			glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, rawvertexsize,

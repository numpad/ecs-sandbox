...
ChunkedTerrain::ChunkedTerrain(vec3 chunkSize) : chunkSize(chunkSize) {
}
...
vec3 ChunkedTerrain::worldPosToLocalChunkPos(vec3 p) const {
	...
	// 0.0001f actually
	p.x = glm::sign(p.x - chunk.x * (chunkSize.x * 2.f)) *
	      glm::mod(glm::abs(p.x - chunk.x * (chunkSize.x * 2.f)), chunkSize.x + 0.0001f);
	p.z = glm::sign(p.z - chunk.y * (chunkSize.z * 2.f)) *
	      glm::mod(glm::abs(p.z - chunk.y * (chunkSize.z * 2.f)), chunkSize.z + 0.0001f);
	return p;
}
...
float ChunkedTerrain::raycastd(vec3 origin, vec3 dir, float max_length) {
	...
	if (dist < max_length) {
		if (sampleValue(origin + dir * max_length) <= 0.f)
			return max_length;
	}
	return -1.0f;
}

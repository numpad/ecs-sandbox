...
void Font::drawString(mat4 uProj, std::wstring str, float x, float y, float scale, vec3 color) {
	...
	for (wchar_t c : str) {
		...
		float w = ch.size.x * scale;
		float h = ch.size.y * scale;
		GLfloat vertices[24] = {xpos, ypos + h, 0.f, 0.f, xpos,     ypos, 0.f, 1.f, xpos + w, ypos,     1.f, 1.f,
		                        xpos, ypos + h, 0.f, 0.f, xpos + w, ypos, 1.f, 1.f, xpos + w, ypos + h, 1.f, 0.f};

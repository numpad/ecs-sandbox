...
void SpritePreviewScene::onRender() {
	...
	glm::mat4 model_source = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(50.0f, 50.0f, 0.0f)),
	                                    glm::vec3(m_texture->get_width() * 4.0f, m_texture->get_height() * 4.0f, 1.0f));
	m_imageShader->operator[]("uProjection") =
	    glm::ortho(0.0f, screen_width, screen_height, 0.0f); // m_camera->getHudProjection();
	...
	glm::mat4 model_preview =
	    glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(350.0f, 50.0f, 0.0f)),
	               glm::vec3(m_texCoords->get_width() * 8.0f, m_texCoords->get_height() * 8.0f, 1.0f));
	m_previewShader->use();
	m_previewShader->operator[]("uProjection") =
	    glm::ortho(0.0f, screen_width, screen_height, 0.0f); // m_camera->getHudProjection();
...
void SpritePreviewScene::reload() {
	...
	delete m_texCoords;
	{
		...
		m_texture->load(width, height, sgl::texture::internalformat::rgba, data, sgl::texture::format::rgba,
...

	{
		...
		m_texCoords->load(width, height, sgl::texture::internalformat::rgba, data, sgl::texture::format::rgba,
		                  sgl::texture::datatype::u8);
		stbi_image_free(data);
	}
